...
export default function Qidiruvdagilar() {
  ...
  const postDataQidiruv = async () => {
    try {
      ...
      formData.append("cathWantedImages", PolisFotosBayonnomaQ);
      formData.append("wantedFingerPrint", PolisfingerBayonnomaQ);
      formData.append("comment", comment.current.value);
      let TokenPPSID = localStorage.getItem("idPPS");
      console.log(TokenPPSID);
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND_API}/api/v1/addCathWanted/${TokenPPSID}`,
        {
          method: "POST",
          headers: {
